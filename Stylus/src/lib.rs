...
impl ContentPurchaseContract {
    ...
    pub fn purchase_content(&mut self, content_type: u64) -> Result<(), Vec<u8>> {
        ...
        let balance_of_call = IERC20::balanceOfCall { account: buyer };
        let call_data = balance_of_call.abi_encode();
        let balance_result = unsafe {
            RawCall::new()
                .call(usdt_addr, &call_data)
        };
        let user_balance = if let Ok(result) = balance_result {
            if result.len() >= 32 {
    ...
    pub fn withdraw(&mut self) -> Result<(), Vec<u8>> {
        ...
        let contract_addr = self.vm().contract_address();
        let usdt_addr = self.usdt_address.get();
        let balance_of_call = IERC20::balanceOfCall { account: contract_addr };
        let call_data = balance_of_call.abi_encode();
        let balance_result = unsafe {
            RawCall::new()
                .call(usdt_addr, &call_data)
        };
        let amount = if let Ok(result) = balance_result {
            if result.len() >= 32 {
                U256::from_be_slice(&result[..32])
            } else {
                U256::ZERO
