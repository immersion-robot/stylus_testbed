//! 개인키 주소가 가진 Token ID들의 waypoint를 B로 변경하는 예제
//! 
//! 사용법:
//!   cargo run --example set_waypoint_b
//!
...
async fn main() -> eyre::Result<()> {
    ...
    // 2. 각 Token ID의 Waypoint를 B로 설정
    println!("2. 각 Token ID의 Waypoint를 B로 설정 중...");
    ...
    for i in 0..token_count_u64 {
        ...
        // Waypoint를 B로 설정
        println!("    → Waypoint B(2)로 변경 중...");
        match contract.set_waypoint(token_id, WAYPOINT_B).send().await {
            Ok(pending) => {
