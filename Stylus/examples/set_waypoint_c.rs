//! 개인키 주소가 가진 Token ID들의 waypoint를 C로 변경하는 예제
//! 
//! 사용법:
//!   cargo run --example set_waypoint_c
//!
...
async fn main() -> eyre::Result<()> {
    ...
    // 2. 각 Token ID의 Waypoint를 C로 설정
    println!("2. 각 Token ID의 Waypoint를 C로 설정 중...");
    ...
    for i in 0..token_count_u64 {
        ...
        // Waypoint를 C로 설정
        println!("    → Waypoint C(3)로 변경 중...");
        match contract.set_waypoint(token_id, WAYPOINT_C).send().await {
            Ok(pending) => {
